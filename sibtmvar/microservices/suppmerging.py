...
class DocumentsMerging:
    ...
    def __init__(self, documents_per_query, conf_file=None, conf_mode="prod"):
        ''' Store the queries '''
        ...
        # Compute the merging
        self.compute(documents_per_query)


    def compute(self, documents_per_query):
        ''' Merge documents for each gene-variant couple and for each collection '''

        # Get list of all identifiers
        list_identifiers = self.getAllIdentifiers(documents_per_query)
